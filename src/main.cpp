...
#if defined(NDEBUG)
# error "Coinfordonation cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0eba3617224368047247c5fb578576eb0484b8e62d36467fa689f9d95e9bbeda");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Coinfordonation: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Coinfordonation Signed Message:\n";
...
{
    // Coinfordonation: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 200000); // Coinfordonation: 200k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Coinfordonation: 3 days
static const int64 nTargetSpacing = 2.5 * 60; // Coinfordonation: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Coinfordonation: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Coinfordonation: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0187520e21574224315289e17d49f55d0fa63923978a0f95ad16b05ece582514");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "17/04/2021 - coinfordonation start from this moment";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1000000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0448bb28fad561be2c0fd2873158315fc085ad468aff9b139120ec26115c0a5b8e71fd6b5f787170f250c79f1461a7c67f49452d2d46f7d2cc4a93884c96c6cb28") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1618646266;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085415493;

        if (fTestNet)
        {
            block.nTime    = 1618646223;
            block.nNonce   = 386476585;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xb1166e4ccfb6996091bb469092b04c24acb56cff8db00bed6ea3ceadc39ea1fc"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc1, 0xb8, 0xdd }; // Coinfordonation: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CoinfordonationMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CoinfordonationMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CoinfordonationMiner(CWallet *pwallet)
{
    printf("CoinfordonationMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("coinfordonation-miner");
    ...
    try { loop {
        ...
        printf("Running CoinfordonationMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CoinfordonationMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CoinfordonationMiner, pwallet));
}
